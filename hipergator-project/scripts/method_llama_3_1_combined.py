#!/usr/bin/env python3
"""
Federated LLM with Meta-Llama-3.1-8B-Instruct
...
print("\n" + "="*70)
print("META-LLAMA-3.1-8B-INSTRUCT FEDERATED COMPARISON")
...
try:
    ...
    print("\nTo fix this:")
    print("1. Run: hf_token login")
    print("2. Go to: https://huggingface.co/settings/tokens")
    print("3. Create a new token and paste it")
...
def aggregate_tokens(token_predictions, aggregation_method="majority"):
    ...
    Args:
        token_predictions: List of (token_id, probability) tuples from each client
        aggregation_method: "majority" or "consensus"
    
    Returns:
        Aggregated token_id
    """
    if aggregation_method == "majority":
        # Token that appears most frequently across clients wins
        token_ids = [t[0] for t in token_predictions]
        return max(set(token_ids), key=token_ids.count)
    
    elif aggregation_method == "consensus":
        # All clients must agree
        first_token = token_predictions[0][0]
        if all(t[0] == first_token for t in token_predictions):
            return first_token
        else:
...
def method_1_centralized(prompt):
    ...
    print(f"   Strategy: All computation at one location")
...
def method_2_federated_majority_vote(prompt):
    ...
    with torch.no_grad():
        for i in range(max_tokens):
            # Each of 3 clients generates a prediction
            client_predictions = []
            ...
            for client_id in range(num_clients):
                ...
                # Communication: broadcast probabilities (50 tokens × 4 bytes)
                comm_bytes += 200
            
            # COMBINE via majority voting
            aggregated_id = aggregate_tokens(client_predictions, "majority")
            
            # Get probability for the combined token
            outputs = model(all_tokens)
            logits = outputs.logits[0, -1, :]
            logits = logits / temperature
            probs = torch.softmax(logits, dim=-1)
    ...
    print(f"   Breakdown: {num_clients} clients × 50 probs × 4 bytes × {max_tokens} tokens")
...
def method_3_optimized_batch_combine(prompt):
    ...
    with torch.no_grad():
        for i in range(max_tokens):
            ...
            for client_id in range(num_clients):
                ...
                client_predictions.append((next_id.item(), prob))
                # Communication: 8-bit quantized (1 byte per prob)
                comm_bytes += 50  # 50 probs × 1 byte (8-bit)
            ...
            # Batch aggregation: send combined tokens every 2 steps
    ...
    print(f"   Time:  {elapsed:.2f}ms")
    print(f"   Comm:  {comm_bytes/1024:.4f} KB")
    print(f"   Breakdown: {num_clients} clients × 50 probs × 1 byte × ~{(max_tokens//batch_size)+1} batches")
    print(f"   Optimizations:")
    print(f"      • 8-bit quantization: 4x smaller (1 byte vs 4 bytes)")
    print(f"      • Batch every {batch_size} tokens: 50% reduction in communication frequency")
...
for i, prompt in enumerate(prompts):
    ...
    if c2 > 0:
        ...
        print(f"     {c2:.4f} KB (Majority Vote) → {c3:.4f} KB (8-bit + Batch)")
...
print(f"\n✅ Llama-3.1-8B-Instruct evaluation complete!")
print(f"\nKey Features:")
