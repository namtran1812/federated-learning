...
scp /Users/namtran/LLM/federated-llm/src/generate_federated_text.py \
    nam.tran1@hpg-login1.rc.ufl.edu:/blue/jie.xu/nam.tran1/federated-llm/src/ 2>&1

echo "[2/4] Submitting SLURM job..."
JOBID=$(ssh nam.tran1@hpg-login1.rc.ufl.edu << 'SSH_COMMAND'
...
# Step 3: Monitor job
echo "[3/4] Job Status:"
ssh nam.tran1@hpg-login1.rc.ufl.edu "squeue -j $JOBID --format='%.18i %.9P %.8j %.8u %.2t %.10M %.6D %R'"
echo ""
...
echo "  ssh nam.tran1@hpg-login1.rc.ufl.edu 'squeue -j $JOBID'"
echo ""
echo "After job completes (~20-30 mins), download results:"
echo "  scp nam.tran1@hpg-login1.rc.ufl.edu:/blue/jie.xu/nam.tran1/federated-llm/results/phase_4_generations.json \\"
